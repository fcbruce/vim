  ...
  if &filetype == 'python' || &filetype == 'sh'
    ...
    if &filetype == 'java'
        ...
        if expand("%<") == "Main"
            ...
            call append(line(".")+52,"    StringBuilder sb = new StringBuilder();")
            call append(line(".")+53,"    while (!(isSpaceChar(b))) { // when nextLine, (isSpaceChar(b) && b != ' ')")
            call append(line(".")+54,"      sb.appendCodePoint(b);")
            call append(line(".")+55,"      b = readByte();")
            call append(line(".")+56,"    }")
            call append(line(".")+57,"    return sb.toString();")
            call append(line(".")+58,"  }")
            call append(line(".")+59,"  ")
            call append(line(".")+60,"  private char[] nextString(int n) {")
            call append(line(".")+61,"    char[] buf = new char[n];")
            call append(line(".")+62,"    int b = skip(), p = 0;")
            call append(line(".")+63,"    while (p < n && !(isSpaceChar(b))) {")
            call append(line(".")+64,"      buf[p++] = (char)b;")
            call append(line(".")+65,"      b = readByte();")
            call append(line(".")+66,"    }")
            call append(line(".")+67,"    return n == p ? buf : Arrays.copyOf(buf, p);")
            call append(line(".")+68,"  }")
            call append(line(".")+69,"  ")
            call append(line(".")+70,"  private int[] nextArray(int n) {")
            call append(line(".")+71,"    int[] a = new int[n];")
            call append(line(".")+72,"    for (int i = 0;i < n;i++) a[i] = nextInt();")
            call append(line(".")+73,"    return a;")
            call append(line(".")+74,"  }")
            call append(line(".")+75,"  ")
            call append(line(".")+76,"  private int nextInt() {")
            call append(line(".")+77,"    int num = 0, b;")
            call append(line(".")+78,"    boolean minus = false;")
            call append(line(".")+79,"    while ((b = readByte()) != -1 && !((b >= '0' && b <= '9') || b == '-'));")
            call append(line(".")+80,"    if (b == -1) throw new InputMismatchException();")
            call append(line(".")+81,"    if (b == '-') {")
            call append(line(".")+82,"      minus = true;")
            call append(line(".")+83,"      b = readByte();")
            call append(line(".")+84,"    }")
            call append(line(".")+85,"    ")
            call append(line(".")+86,"    while (true) {")
            call append(line(".")+87,"      if (b >= '0' && b <= '9') {")
            call append(line(".")+88,"        num = num * 10 + (b - '0');")
            call append(line(".")+89,"      } else { ")
            call append(line(".")+90,"        return minus ? -num : num;")
            call append(line(".")+91,"      }")
            call append(line(".")+92,"      b = readByte();")
            call append(line(".")+93,"    }")
            call append(line(".")+94,"  }")
            call append(line(".")+95,"  ")
            call append(line(".")+96,"  private long nextLong() {")
            call append(line(".")+97,"    long num = 0;")
            call append(line(".")+98,"    int b;")
            call append(line(".")+99,"    boolean minus = false;")
            call append(line(".")+100,"    while ((b = readByte()) != -1 && !((b >= '0' && b <= '9') || b == '-'));")
            call append(line(".")+101,"    if (b == -1) throw new InputMismatchException();")
            call append(line(".")+102,"    if (b == '-'){")
            call append(line(".")+103,"      minus = true;")
            call append(line(".")+104,"      b = readByte();")
            call append(line(".")+105,"    }")
            call append(line(".")+106,"    ")
            call append(line(".")+107,"    while (true) {")
            call append(line(".")+108,"      if (b >= '0' && b <= '9') {")
            call append(line(".")+109,"        num = num * 10 + (b - '0');")
            call append(line(".")+110,"      } else {")
            call append(line(".")+111,"        return minus ? -num : num;")
            call append(line(".")+112,"      }")
            call append(line(".")+113,"      b = readByte();")
            call append(line(".")+114,"    }")
            call append(line(".")+115,"  }")
            call append(line(".")+116,"  ")
            call append(line(".")+117,"  private boolean FCBRUCE = System.getProperty(\"FCBRUCE\") != null;")
            call append(line(".")+118,"}")
        else
